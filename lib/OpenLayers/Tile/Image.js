...
OpenLayers.Tile.Image = OpenLayers.Class(OpenLayers.Tile, {
     ...
     */
     positionImage: function() {
        ...
        OpenLayers.Util.modifyDOMElement(this.frame, {
            pixel: this.position, size: this.size});

        var imageSize = this.layer.getImageSize(this.bounds); 
        if (this.layerAlphaHack) {
            OpenLayers.Util.modifyAlphaImageDiv(this.imgDiv, {
                size: imageSize, imageURL: this.url});
        } else {
            OpenLayers.Util.modifyDOMElement(this.imgDiv, {size: imageSize});
    ...
    initImgDiv: function() {
        if (this.imgDiv == null) {
            ...
            if (this.layerAlphaHack) {
                this.imgDiv = OpenLayers.Util.createAlphaImageDiv({pixel: offset,
                    size: size, position: "relative", delayDisplay: true});
            } else {
                this.imgDiv = OpenLayers.Util.createImage({pixel: offset,
                    size: size, position: "relative", delayDisplay: true});
            }
        ...
        if (ratio != this.lastRatio) {
            if (this.layer.transitionEffect == 'resize') {
                ...
                OpenLayers.Util.modifyDOMElement(this.backBufferTile.imgDiv, {
                    pixel: imageOffset, size: imageSize});
